...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if classifier_free:
            x_new = x.repeat(2, 1, 1, 1)
            x_new[B:, :-1, ...] = th.zeros_like(x[:, :-1, ...])
            t_new = t.repeat(2)
        
        # model_output is epsilon - noise predicted from noisy image
        model_output = model(x_new, self._scale_timesteps(t_new), **model_kwargs) if classifier_free \
            else model(x, self._scale_timesteps(t), **model_kwargs)
        ...
        if isinstance(model_output, tuple):
            ...
            if classifier_free:
                w = 5.0
                model_output = (1 + w) * model_output[:B] - w * model_output[B:]
                cal = (1 + w) * cal[:B] - w * cal[B:]
        ...
        sqrt_one_minus_alphas_cumprod = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, model_output.shape)
        score = -model_output / sqrt_one_minus_alphas_cumprod
    ...
    def p_sample(
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "cal": out["cal"], "score": out["score"]}
    ...
    def p_sample_loop_known(
        ...
        return final["sample"], x_noisy, img, final["cal"], cal_out, scores, samples
